...
import { FaBookmark, FaUserCog, FaSignOutAlt, FaTimes } from 'react-icons/fa';
...
function SideBar() {
    ...
    const menuItems = [
        ...
        },
        {
            ...
            label: 'My profile',
            icon: <RiUser3Fill />,
            path: '/worker/dashbord/myProfile'
        }
    ];

    return (
        <div className="h-full overflow-y-auto scrollbar-hide w-[280px] bg-gray-100 dark:bg-[#0f172a] text-black dark:text-white border-r-2 border-blue-600 dark:border-purple-600 flex-shrink-0 relative">

            {/* Sidebar Header */}
            <div className="p-4 border-b border-gray-300 dark:border-gray-700 relative">
                <div className="text-xl flex justify-center items-center gap-4 text-blue-600 dark:text-purple-500">
                    ...
                    >
                        <img
                            className="w-15 h-15 object-cover rounded-full border-2 border-blue-600 dark:border-purple-500"
            ...
            <nav className="p-3 space-y-1">
                {menuItems.map((item) => (
                    <NavLink
                        ...
                        to={item.path}
                        className={({ isActive }) =>
                            `w-full flex items-center gap-3 px-4 py-5 mt-2 rounded-lg transition-all duration-200 text-sm font-medium
                             ${isActive
